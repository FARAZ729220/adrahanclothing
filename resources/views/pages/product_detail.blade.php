{{-- <x-layout :title="$product->name . ' - Adrahan Clothing'" :description="$product->description ?: 'Shop ' . $product->name . ' at Adrahan Clothing.'">
    ...
    <section class="product-detail-section bg-white" style="padding: 100px">
    ...
    <script>
        ...
        if (typeof fbq === 'function') {
            fbq('track', 'ViewContent', {
                ...
                content_category: '{{ $product->category->name ?? 'Clothing' }}',
...
<x-layout :title="$product->name . ' - Adrahan Clothing'" :description="$product->description ?: 'Shop ' . $product->name . ' at Adrahan Clothing.'">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const maxStock = {{ $maxStock }};
            const finalPrice = {{ json_encode((float) $finalPrice) }};
            const productId = @json((string) $product->id);
            const productName = @json($product->name);
            const categoryName = @json($product->category->name ?? 'Clothing');
            ...
            form?.addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    if (typeof data.cart_count !== 'undefined' && typeof window.setCartBadge ===
                        'function') {
                        window.setCartBadge(data.cart_count);
                    }
